...
- Two tests: one relationship-driven candidate-selection flip, one memory-driven candidate-selection flip using a deprivation memory (per §3 step 4's correction — `memoryContributions` reads deprivation memories only; a relational memory cannot reach weighting without new production code).
- One additional, explicitly named test pinning that gap: a relational memory formed via a real tick produces no candidate-selection change today.
...
- No required decision (D1–D8) changes. One adjacent fact measured during implementation remains recorded in PR #162 rather than folded into this plan, because it is a calibration observation, not a plan change: Comfort's provisional `comfortAffinityDeltaPerTick` cannot reach ADR-16 relational-memory significance even across a whole free period (the sibling of Finding 4's question for Confrontation).
...
**Risks:** Unchanged from prior revisions — the relationship/memory candidate-flip scenarios (§3) remain the least mechanically specified part of this plan (Finding 3); the Assist-checkbox question (Finding 1) still blocks Issue #119's own full closure regardless of how well this slice executes, which is a Human decision, not an engineering risk.
**Follow-up Tasks:** Codex re-review of this correction PR (#164) at its current head. Human sign-off on the correction (scoped to this PR only — does not reopen the already-closed plan-approval or Finding-1 gates from PR #161). Once merged, PR #162 (already implementing the corrected behavior) can proceed to its own review/merge on the corrected plan text.

**No `prototype/src` changes** — this PR contains only the plan document. Nothing in it is binding until the plan is Human-approved, and — separately — until Finding 1 (§11) receives its own Human ruling on the Assist checkbox.
