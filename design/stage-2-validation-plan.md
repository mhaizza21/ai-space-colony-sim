...
**Version:** 0.3.1 (revision 4 — corrects §10's inverted Comfort Social-crediting bullet, found during implementation; see Changelog)
**Phase:** Phase 3 — Stage 2 Slice 9
**Status:** Draft — revision 3, awaiting Codex re-review at the new head, then Human approval (`docs/ai-workflow/operating-model.md` Design → Human Approval gate)
...
**v0.3.1 note — the literal save-string half of that shape is currently unachievable, for a reason outside this plan.** Implementation found that `serialize()` is not byte-stable across a load round trip once a colonist holds memories: it is `JSON.stringify`, so it records key *insertion* order, and `deserialize()` rebuilds a `MemoryEntry` in a different key order than `memory.ts` uses when forming one. The states are identical and `verifyReplay` matches; only the encodings differ. Slice 9 therefore asserts the continuation contract on the **decoded** state (plus summary and replay match), which is what "reaches the same state" means, and pins the encoding defect separately. Tracked as issue #163; when that is fixed, this section's literal string comparison becomes usable again and should be restored.
...
**v0.3.1 — revision 4, correcting an inverted consequence claim found during implementation (Issue #160 T4 / PR #162):**
...
- No required decision (D1–D8) changes. Two adjacent facts measured during implementation are recorded in PR #162 rather than folded into this plan, because both are calibration observations rather than plan changes: Comfort's provisional `comfortAffinityDeltaPerTick` cannot reach ADR-16 relational-memory significance even across a whole free period (the sibling of Finding 4's question for Confrontation), and `memoryContributions` reads deprivation memories only, so §3 step 4's *relational*-memory-driven flip is not reachable without new production code — the memory-driven flip uses a deprivation memory instead.
...
**Validation:** Every "gap" and "already covered" claim re-verified against the actual test suite for this revision, not just the prior revision's own claims — re-read the exact `execution.test.ts:270-275` lines Codex cited before editing, to confirm the correction rather than take the finding on faith. `git diff --check` clean; `node tools/ai-workflow/validate-workflow-pack.mjs .` run and passing (see PR).
...
**Follow-up Tasks:** Codex re-review of this exact revision (PR #161). Human ruling on Finding 1 (Assist checkbox wording) before or alongside plan approval. No implementation, no Assist work, no Confrontation/Comfort recalibration until the plan is Approved and Human-approved.
