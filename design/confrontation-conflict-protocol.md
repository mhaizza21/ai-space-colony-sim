...
**This is hook-only, not ADR-18 D8's scaling behavior, and that gap requires explicit Human acceptance before implementation.** ADR-18 D8 names Stress Response traits (Resilient/Volatile) "scaling each side's accumulation" as part of Confrontation's specified behavior. This slice does not deliver that scaling: no trait multiplies the channel by anything other than `1`, and `"volatile"` does not exist as a trait at all, so Confrontation's stress spike is, in this slice, trait-blind in practice even though the hook it will eventually run through is present. Implementation must not describe or report this slice as having delivered ADR-18 D8's Resilient/Volatile scaling — only the hook. The Human gate reviewing this design must accept this narrower scope explicitly (Finding 1), not infer full D8 delivery from the hook's presence.
...
- **Phase 4 (condition & trigger detection), new sub-step — exact insertion point in `tick.ts`:** `tick.ts` builds the shared observation basis first (`sharedObservations`, the `ObservableColonist[]` array — gains `moduleId` per D1b, no change to its construction point), then immediately runs the existing interruption / suspension-resolved detection block (the code the file's own comment marks `"design D2 phase 4, continued"`, ending with `suspensionOverflow`/`suspensionResolved` event emission). This design's new `conflictDetection.ts` sub-step runs **immediately after that interruption/suspension-resolved block and before Phase 5's decision loop begins** (the loop the file's own comment marks as generating candidates "fresh per deciding colonist, since Phase 4's tier-check candidates above..."). Concretely: interruption/suspension detection may change a colonist's `Execution`/suspended-goal state this same tick; Confrontation's shared-module conjunct (D1b) reads `moduleId` from the fixed pre-Phase-5 `sharedObservations` basis, not from any post-interruption live execution state, so running after interruption/suspension detection does not leak a same-tick interruption into eligibility — the conjunction inputs are the fixed basis plus each colonist's own already-computed Phase-3 stress, neither of which interruption/suspension detection touches. `conflictDetection.ts`'s pure function evaluates every eligible pair (D1's conjunction) in the fixed pair order (D10), draws PRNG for each eligible pair (D4), and returns the set of firing pairs plus each one's evidence (both relationship states, combined stress, shared module id). `tick.ts` then, for each firing pair: applies the relationship delta (D5), applies the stress spike directly to each participant's `StressState` (a discrete addition to `level`, clamped exactly as `evaluateStress`'s own `clamp01` already does — not routed back through a second `evaluateStress` call this same tick, since Phase 3's per-colonist stress evaluation already completed; the spike is a direct, attributed, one-shot mutation applied once here, decomposable in the same `stressEvaluated`-style event), sets `inConflictUntilTick` for both, and emits the new `confrontationOccurred` event (D9).
...
- **Save/load:** one new `ColonistRuntime` field (`inConflictUntilTick: number | null`), one new `StressChannelId` member, and one new `TickEvent` variant (below) — all three governed by the drafted ADR-25 (ADR-22 D1/D6 amendment), which is now the single authority for their persisted validation-site enumeration and load-rejection rules. Load rejects a non-integer or negative `inConflictUntilTick`. **A non-null value `<=` the loaded document's own `clock.tick` is valid, not rejected** — it is ordinary, expected state: the display window has already lapsed by the time the save was taken, and `ambientStateFor`'s `currentTick < inConflictUntilTick` check (D6 above) already reads it correctly with no special-casing. `inConflictUntilTick` has no cross-colonist pairing invariant to enforce at load time, unlike Comfort's one-comforter-per-recipient rule — it is single-colonist-scoped state, set once by the firing code path, never resumed or re-derived.
...
- **Event log:** one new `TickEvent` variant, `{ kind: "confrontationOccurred"; colonistAId; colonistBId; sharedModuleId; combinedStress; severity: "hostile" | "fractured" }` — `colonistAId`/`colonistBId` in the canonical `(min, max)` order D10 fixes. This is a genuinely new, persisted validation site (widening `tick.ts`'s `TickEvent` union and `serialization.ts`'s `readTickEvent` exhaustive switch by one member), governed by ADR-25 D2a alongside the `ColonistRuntime`/`StressChannelId` changes, at the same `eventLog[].event` site category ADR-24 D3 site 3 already established for `socialOfferCreated`'s widened `action` list. It is emitted in addition to, not instead of, the existing `stressEvaluated` event: the stress side reuses `stressEvaluated`'s existing `contributions` array (the new channel is one more decomposed contribution there, mirroring how Comfort's relief needed no new event kind), while `confrontationOccurred` separately records the firing itself (the pair, the shared module, the combined stress that triggered it, and which severity threshold applied) for inspector/replay traceability that `stressEvaluated` alone cannot carry (it has no notion of "pair" or "module").
- **Inspector:** the existing per-colonist summary already exposes `ambientState` (now `inConflict`-capable) and stress with source breakdown (now showing the new channel); no new inspector field is required beyond what already exists generically. A reviewer may want the raw `inConflictUntilTick` value, or the `confrontationOccurred` event itself, surfaced directly for debugging — flagged as an implementation-freedom choice, not a design requirement.
...
1. **Human-acceptance item — hook-only trait modulation, no `"volatile"` trait, in this slice.** This design wires the `stressResponseConflictMultiplier` hook (§6/D5) but adds no trait content: every currently-defined trait returns `1`, and ADR-18 D8's Resilient/Volatile *scaling* behavior is not delivered by this slice, only the hook it will eventually run through. Issue #155's own scope note ("trait modulation hooks already authorized") reads as authorizing the hook, not trait authorship, and DQ-T1 (the canonical trait list) has consistently been treated as its own separate authority in every prior Stage 2 design. **The Human gate must explicitly accept this narrower scope** — hook-only, no Volatile trait — rather than approve the design under an assumption that D8's scaling ships with it.
2. **Human-acceptance item — the shared-module conjunct structurally exempts any colonist currently in a social task or idling** (moduleId null) from ever being a Confrontation participant, even if their relationship/stress facts would otherwise qualify. This may be an acceptable Stage 2 scale limitation (matching Comfort/Assist's own free-period/task-vocabulary limitations, `design/comfort-assist-protocol.md` Finding 2) or may be judged to need a broader "proximity" signal than task-module co-location. **This narrowness must be explicitly accepted by the Human gate before implementation**, since widening it later would touch `WorldSnapshot`/`ObservableColonist` further and is not a change this design's approval should be read as pre-authorizing.
...
- Save/load round-trip of a state with a colonist mid-`inConflict` window, and separately a colonist whose `inConflictUntilTick` has already elapsed (`<=` the loaded clock), both preserve the field bit-identically and both load successfully (elapsed is valid, not rejected — see D9/ADR-25 D4). Load rejects a non-integer/negative `inConflictUntilTick`.
- A `confrontationOccurred` event round-trips bit-identically; load rejects one with a self-pair, out-of-canonical-order ids, an unknown `sharedModuleId`, an out-of-range `combinedStress`, or an unrecognized `severity` (ADR-25 D2a/D4).
...
**v0.2.0 — revision 2, closing Codex design/architecture review round 1 (Changes Required):**
- **B1 (blocker):** `confrontationOccurred` was named as a new `TickEvent` variant (§10) but not governed by ADR-25's persisted-validation-site enumeration. Fixed: ADR-25 now has D2a governing this event's full payload schema and load-rejection rules, and its site is added to D3's table (now three sites) and Required Invariants (new #19). §10 here updated to point at ADR-25 D2a as the authority.
- **B2 (blocker):** §9 (D8) previously proposed rejecting an `inConflictUntilTick` value `<=` the loaded clock (mirroring ADR-21 D5), while §10 (D9) and ADR-25's own D4 said no such cross-field invariant exists — an internal contradiction. Fixed by choosing one rule: an expired non-null `inConflictUntilTick` is valid, inert state, never rejected against the loaded clock (ADR-25's Option F documents why the ADR-21 D5 analogy doesn't hold). §9 and §10 here, and ADR-25's D4/Invariants 15/18, now state this identically.
- **W1:** §8 (D7)'s phase-placement wording was ambiguous about where the Confrontation sub-step runs relative to `tick.ts`'s existing phases. Fixed: now states explicitly that it runs after the shared observation basis is built and after the existing interruption/suspension-resolved detection block, and before Phase 5's decision loop, with the same-tick non-observability reasoning spelled out.
- **W2:** §6 (D5) and Finding 1 now explicitly flag that this slice ships hook-only trait modulation (no Resilient/Volatile scaling, no `"volatile"` trait) and that the Human gate must accept this narrower scope explicitly rather than infer full ADR-18 D8 delivery from the hook's presence.
- **W3:** Finding 2 (the shared-module proxy's narrowness) is now stated as an explicit Human-acceptance item, not merely a flagged ambiguity.
- Test matrix (§14) extended: expired-`inConflictUntilTick`-is-valid round-trip test; `confrontationOccurred` round-trip and load-rejection tests.
- No behavioral decision (D1–D7, D10) changed in substance; ADR determination (D8/D9) is unchanged in conclusion (still an ADR-22 D1/D6 amendment) and only gains the D2a event-governance scope.

**v0.1.0 — initial draft**, per Issue #155.
